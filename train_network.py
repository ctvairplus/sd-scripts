    ...
    def train(self, args):
        ...
        for epoch in range(num_train_epochs):
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                logs = {"avr_loss": avr_loss, "current_timestep": torch.to_numpy(timesteps[0])}  # , "lr": lr_scheduler.get_last_lr()[0]}
                progress_bar.set_postfix(**logs)

                if args.rlessucb:
                    agent = args.rlessucb
                    agent.new_observation(torch.to_numpy(timesteps[0])//100, torch.to_numpy(loss))
